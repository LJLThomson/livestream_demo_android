...
import cn.ucai.live.data.model.EmojiconExampleGroupData;
...
//        if (username.equals(PreferenceManager.getInstance().getCurrentUsername()))
//            return getUserProfileManager().getCurrentUserInfo();
...
//                    }
    ...
    public void asyncFetchContactsFromServer(final EMValueCallBack<List<String>> callback) {
        ...
        model.loginContact(appContext, EMClient.getInstance().getCurrentUser(), new OnCompleteListener<Result>() {
            @Override
            public void onSuccess(Result result) {
                if (result != null){
                    User[] users = new Gson().fromJson(result.getRetData().toString(),User[].class);
                    List list = Arrays.asList(users);
                    ArrayList<User> user_list = new ArrayList<User>(list);
...
//                      将其加到联系人列表中
                        ...
                        dao.saveAppContactList(user_list);
